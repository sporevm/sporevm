...
| `spore build` Dockerfile subset, `.dockerignore`, and build-context hashing | local build contexts and caller-selected Dockerfiles | M1 supports only a fail-closed Dockerfile subset (`FROM`, shell-form `RUN`, flag-less `COPY`, `ENV`, `ARG`, `WORKDIR`, `CMD`) and named OCI-layout build contexts; unsupported syntax, context escape attempts, unsupported ignore patterns, unsupported COPY globs, missing sources, and special-file COPY inputs fail before cache lookup or image publication. Dockerfile and `.dockerignore` parsers carry fuzz targets in the introducing slice |
