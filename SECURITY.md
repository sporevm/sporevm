...
| x86-64 KVM PIO exit envelope and finite board policy | guest-issued x86 port I/O reflected through `KVM_EXIT_IO` | The host-only Stage 0a.3 boot harness validates the mapped `kvm_run` length, direction, width, decoded u16 port, nonzero count, checked aggregate byte length capped at 4KiB, data offset conversion, and the complete data range before reading or completing an exit. Its native-evidence policy accepts only one-byte, count-one accesses: writes of `0x0f` to CMOS index port `0x70`, writes of `0x0a` or `0x00` to CMOS data port `0x71`, reads of keyboard-controller status port `0x64` which return zero, and writes of reset command `0xfe` to port `0x64` which terminate as `guest_reset`. Direction, port, width, count, and value are all policy inputs; every other tuple fails the harness before exit completion. The bounded envelope and finite policy have focused malformed/nearby-input tests and continuous fuzz coverage. Raw `KVM_EXIT_SHUTDOWN` remains an unclassified fatal exit and cannot acquire reset or poweroff meaning. This harness is not reachable from the product runner; Slice 2a must preserve the same fail-closed table when the reviewed x86 board enters the product path. |
...
| x86-64 profile round-trip mailbox and task-local state | guest-owned probe page, KVM CPU state, and a task-selected local state path | The host-only Stage 0b.2 harness maps one separate 4 KiB probe memslot outside advertised guest RAM and accepts only exact aligned 32-bit `CAPT` and `RSTR` writes at two stage-private generation-page offsets. It drains the capture MMIO completion under `immediate_exit` before reading state, then strictly validates the fixed 512-byte little-endian mailbox: magic, version, phase, nonce, CPUID selectors, XCR0/AVX contract, clock ranges, x87/SSE/AVX patterns, reserved bytes, and checksum are all bounded and fuzz covered. The process-boundary file is evidence-only, not a spore or compatibility claim. It encodes registers, segments, CPUID, XCRs, architectural XSAVE bytes, explicit MSRs, TSC/clock fields, mailbox, and at most 64 MiB RAM field by field with a SHA-256 footer; it never serializes raw KVM structs or padding. Capture creates a private no-follow file without replacement, restore requires a private regular single-link file owned by the caller, and the decoder rejects unknown versions/state bits, oversized counts, noncanonical ordering, length mismatch, trailing data, or checksum failure before creating a VM. Only the BSP runs, the second board vCPU must remain reset, and full LAPIC/event/MP/irqchip/PIT portability remains fail-closed until Stage 0b.3. |
