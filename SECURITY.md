...
| Bundle metadata, chunkpack index, pack segments, and pull/push URIs | peers, registry, disk, S3 | `bundle.json`, `rootfs.index.json`, and chunkpack index parsing are fuzzed; unpack/pull only accept canonical metadata paths, canonical child ids, canonical pack paths, verified rootfs artifact paths, absolute undecoded `file://` pull sources, and digest-pinned `s3://...@sha256:<bundle>` pull sources. S3 pull downloads only the canonical files named by validated metadata, verifies the canonical bundle digest before materialization, then verifies segment SHA256 plus logical BLAKE3 chunk IDs before writing chunks |
