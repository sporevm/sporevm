...
Credential mediation belongs to consumers (for example cleanroom's host
gateway). Values explicitly supplied through per-exec `--env` necessarily
transit the monitor and guest request, but SporeVM does not copy them into host
runtime metadata, saved lifecycle metadata, or manifest fields. As with any
guest process environment, their bytes can remain in guest memory or workload
files and therefore can enter a later save or bundle; `--env` is not a secret
broker. OCI `Config.Env` is image configuration rather than a secret channel;
named image defaults are persisted in `exec_defaults`, so images must not put
credentials there.
...
|---|---|---|
| OCI runtime command and user metadata | OCI image configuration persisted by the rootfs builder | Fresh image-backed run and create compose bounded `Entrypoint` and `Cmd` arrays before guest boot, retain the existing 16-argument and per-argument byte limits, and fail when the effective command is empty. Explicit caller argv replaces only `Cmd`; `spore run --entrypoint` may replace `Entrypoint` explicitly, while saved-spore, restore, attach, and later named-exec paths do not reapply command defaults. `User` accepts only unambiguous root user and root-group spellings until guest credential switching exists, with focused unit and fuzz coverage for the validator. |
| Native image archive | immutable CI or object-store artifact supplied to `spore image unpack` | The client requires the expected SHA-256 of the complete gzip archive, expected native BLAKE3 image identity, and an explicit `linux/arm64` or `linux/amd64` platform. The strict USTAR reader accepts only the canonical manifest, config, index, sorted distinct object closure, and terminal zero blocks; it rejects alternate order, non-regular or extra entries, malformed headers, bad checksums, truncation, trailing bytes, aggregate bound violations, platform mismatch, native-image mismatch, and object length or BLAKE3 mismatch. It hashes the same open file it parses and refuses any outer or inner identity mismatch before cache publication. All bytes stage privately outside the cache lock. The existing CAS transaction re-verifies objects and the index, marks complete storage, and publishes image metadata plus the local ref last. Header parsing has continuous fuzz coverage, and round-trip tests prove digest, identity, and architecture substitution leave no ref. The archive contains no saved-machine state and does not share the bundle parser or authority model. |
| Image gateway platform index | explicitly configured gateway response and typed OCI platform metadata | The canonical JSON parser accepts at most 64 KiB and one or two sorted `linux/amd64` or `linux/arm64` descriptors, and rejects unknown or duplicate fields, alternate formatting, unsupported platforms, non-lowercase fixed-length SHA-256 or BLAKE3 digests, duplicate platforms, and trailing input. The gateway source selector, direct registry pulls, and local OCI-layout imports share one platform-selection authority. It ignores unrelated operating systems, architectures, missing platform metadata, and non-image media types, but rejects ambiguous normalized matches or an unsupported variant for the requested platform. Selection never falls back to another architecture, and every parser path is fuzz or unit tested. The current HTTP proof requires HTTPS except for an explicit `127.0.0.1` or `[::1]` insecure fixture flag, rejects userinfo, queries, fragments, non-root paths, redirects, and invalid repository names, and adds no ambient gateway lookup or runtime dependency. |
| x86-64 Linux direct-boot planner | host-selected bzImage, initrd, command line, and RAM size | The fresh-only x86 VM's planner requires a complete Linux 2.10 setup header before reading its relocation fields, bounds every setup and payload offset, requires a nonzero `init_size` large enough for the loaded payload, and validates the relocatable-kernel alignment as a bounded power of two. It computes the separate load and runtime windows with the Linux boot protocol algorithm using checked arithmetic, bounds both within the single low-RAM slot, and places a nonempty initrd outside both. Command-line length excludes its separately checked terminator, absent or explicitly empty initrds produce zero ramdisk fields, and malformed inputs fail before guest RAM writes. Arbitrary raw images, header-derived placement fields, command lines, initrd lengths, and RAM sizes have continuous fuzz coverage alongside focused malformed-input tests. Product routing is limited to the reviewed fresh-run profile: one vCPU, explicit 512 MiB memory, and no rootfs, network, build, or capture path. |
| x86-64 KVM PIO exit envelope and finite board policy | guest-issued x86 port I/O reflected through `KVM_EXIT_IO` | The fresh-only x86 VM validates the mapped `kvm_run` length, direction, width, decoded u16 port, nonzero count, checked aggregate byte length capped at 4KiB, data offset conversion, and the complete data range before reading or completing an exit. Its policy accepts only one-byte, count-one accesses: writes of `0x0f` to CMOS index port `0x70`, writes of `0x0a` or `0x00` to CMOS data port `0x71`, reads of keyboard-controller status port `0x64` which return zero, writes of reset command `0xfe` to port `0x64` which terminate as `guest_reset`, and the exact `0x80 <- 0x00` legacy I/O-delay no-op observed while the Stage 0b.3 candidate initialized the in-kernel PIC. Direction, port, width, count, and value are all policy inputs; every other tuple fails before exit completion. The bounded envelope and finite policy retain their focused malformed/nearby-input tests and continuous fuzz coverage after the mechanical move from the harness. Raw `KVM_EXIT_SHUTDOWN` remains an unclassified fatal exit and cannot acquire reset or poweroff meaning. Product routing is limited to the reviewed fresh-run profile: one vCPU, explicit 512 MiB memory, and no rootfs, network, build, or capture path. |
| x86-64 KVM MMIO exit routing and board lifecycle control | guest-issued MMIO reflected through `KVM_EXIT_MMIO` | The fresh-only x86 VM requires a complete mapped `kvm_run` MMIO envelope, accepts only naturally aligned 1-, 2-, or 4-byte accesses wholly contained in one frozen virtio-mmio window, permits the generation-device window to additionally accept aligned 8-byte accesses, and rejects every other address, width, overflow, boundary crossing, or truncated envelope before dispatch. The x86 board alone decodes its generation-page poweroff doorbell: an aligned 32-bit write of `0x46464f50` at offset `0x020` terminates as `guest_off`; any write touching that register with another width, placement, or value fails closed, while reads touching it return zero and have no side effect. The shared SPGN v1 device remains unchanged and inert for the board-owned control range. The bounded router and exact stateless control retain focused malformed/nearby-input tests and continuous fuzz coverage after the mechanical move from the harness, including 8-byte laundering rejection. Empty fixed slots expose device ID zero and no queues. Product routing is limited to the reviewed fresh-run profile: one vCPU, explicit 512 MiB memory, and no rootfs, network, build, or capture path. |
| x86-64 profile round-trip mailbox and task-local state | guest-owned probe page, KVM CPU/device state, and a task-selected local state path | The host-only Stage 0b.3 harness maps one separate 4 KiB probe memslot outside advertised guest RAM and accepts only exact aligned 32-bit `CAPT` and `RSTR` writes at two stage-private generation-page offsets. It drains the capture MMIO completion under `immediate_exit` before reading state, then strictly validates the fixed 512-byte little-endian mailbox: magic, version, phase, nonce, CPUID selectors, XCR0/AVX contract, clock ranges, x87/SSE/AVX patterns, reserved bytes, and checksum are all bounded and fuzz covered. The process-boundary file is evidence-only, not a spore format; its named profile is approved only for same-host restore. Codec v3 encodes exactly two independent normalized per-vCPU inventories: configured CPUID, registers and segments, XCRs, exactly 832 architectural XSAVE bytes, the exact MSR inventory, TSC frequency/offset, LAPIC, vCPU-event, debug, and MP state. VM clock, PIC, IOAPIC, PIT2, mailbox, and at most 64 MiB RAM remain VM-wide. It never serializes raw KVM structs or transfer-buffer padding; XSAVE metadata and reserved header bytes must agree with the normalized fields, and capture rejects a nonzero host-buffer tail. Capture also rejects other nonzero KVM reserved fields, active SMM/SMI or triple-fault state, unknown event/debug/PIT flags, and unsupported xstate before publication. Capture creates a private no-follow file without replacement; restore requires a private regular single-link file owned by the caller, rejects malformed or non-candidate state before VM creation, checks host compatibility before applying state, applies every VM-wide and per-vCPU state class before the first `KVM_RUN`, and verifies normalized SET/GET readback. Only the BSP runs and the second board vCPU must remain reset. |
| Image gateway image manifest, config, rootfs-index, and object closure | explicitly configured gateway response | The canonical JSON manifest parser accepts at most 64 KiB, rejects unknown or duplicate fields, alternate formatting, unsupported platform or rootfs device/storage values, malformed lowercase fixed-length digests, control-bearing provenance, inconsistent index/base identities, invalid OCI provenance, impossible object summaries, and trailing input, and carries a fuzz target. Config and rootfs-index inputs retain their 64 MiB bounds and the strict config parser is separately fuzzed. Verification binds the selected platform-index descriptor to the manifest SHA-256 transport digest, platform, native image digest, and matching optional source generation; requires strict canonical config bytes with matching SHA-256 transport and BLAKE3 config digests; requires config OS/architecture to match; parses the rootfs index through the existing canonical full-coverage and BLAKE3 authority; checks exact index bytes, unique object count, and object bytes including a short final chunk; and recomputes native image identity. Before object transfer, the eager proof caps the selected image at 16 GiB logical size, 65,536 distinct nonzero objects, and 4 GiB of nonzero payload. Every distinct nonzero object is fetched only below the selected repository and immutable manifest, with its exact index-derived length. Network I/O completes in private staging before the cache lock; the existing repairing CAS installers re-verify BLAKE3 bytes under that lock, the full storage closure is checked before its completeness stamp, and image metadata plus the mutable local ref publish last. Failed pulls leave no reachable image authority. This proof has no authentication and is not a production gateway client. |
| Image gateway repository-bound object authorization | shared service conformance fixture over authenticated principals, repository grants, immutable manifests, and rootfs-index object closures | The data-plane contract exposes only `GET` beneath an authorized repository and immutable manifest. Missing authentication returns `401`; after authentication, missing or denied repositories, missing manifests, and missing or unreachable objects all return the same empty `404`. Generic `HEAD`, repository-independent object lookup, and malformed routes also return `404`. The normative fixture gives two principals separate repositories backed by the same logical object descriptors, proving that physical dedupe does not grant cross-repository visibility. Its manifest transport digests, rootfs-index identities, object sets, and exact object lengths are derived from the canonical protocol fixtures on every contract test run. This is test-only service conformance and adds no authentication or object-serving path to the SporeVM binary. |
| Image gateway attachment records and subject lists | future authenticated gateway response | Canonical JSON attachment records and subject lists are each limited to 64 KiB and reject unknown or duplicate fields, alternate formatting, unsupported kinds or artifact types, malformed SHA-256 digests, noncanonical media types, invalid UTF-8, trailing input, unsorted descriptors, and duplicate descriptors. Artifact descriptors are nonempty and capped at 64 MiB; subject lists contain at most 256 descriptors. Verification binds the requested immutable image-manifest subject to the returned list, then binds each listed record's exact SHA-256 transport digest and artifact type to that subject and verifies the artifact's exact length and SHA-256 digest before its bytes can be consumed. Records and lists are data-only, carry fuzz coverage, enter no native image identity, create no mutable attachment-tag convention, and add no upload, storage, authorization, retention, or runtime authority. |
...
| Guest memory access during dirty scans | guest | KVM dirty-log and HVF write-protect harness paths have landed; dirty pages plus VMM-originated virtio writes are coalesced to fixed 2MiB chunks, zero chunks are elided, and non-zero chunks are BLAKE3-addressed before being recorded in the manifest. Elastic captures currently use the bounded full-scan path over the maximum mapping so hotplugged pages cannot escape the portable chunk authority. |
...
| Spore manifest decode | registry, disk | manifest v2/v3 fixed-memory and v4/v5 elastic-memory parsing/validation are fuzzed; v0/v1 fail with a format-too-old error, and unknown versions or malformed manifests fail closed. Elastic state is normalized data, never raw KVM or Hypervisor.framework structures. Validation binds initial RAM to `platform.ram_size`, bounds the maximum elastic region to 8192 2MiB blocks, requires requested/captured sizes within that region, checks sorted non-overlapping plugged ranges with checked arithmetic, recomputes captured bytes, requires exactly one virtio-mem transport, and rejects the elastic version/state mismatch before mapping guest memory. |
...
| Manifest-bound chunked rootfs storage descriptor and disk index | manifest, registry, disk, bundle, peers | `rootfs.storage` is parsed as a separate storage authority from OCI provenance, requires `chunked-ext4-rootfs-v0`, BLAKE3, exact `rootfs/blake3` namespace, matching rootfs device binding, 64KiB disk chunks, logical size matching the ext4 artifact size, `base_identity == index_digest`, and `rootfs.artifact.digest == index_digest`; writable disks use `disk.kind: "chunk-index-disk-v0"` with the same descriptor fields and name their versioned index by `disk.base`; index inputs remain capped at 64 MiB and are BLAKE3-checked against the descriptor before parse. Readers preserve canonical v1 compatibility, while current writers use v2 packed contiguous digest ranges and counted zero ranges. The v2 decoder rejects unknown or duplicate fields, malformed or uppercase packed digests, empty, adjacent, overlapping, overflowing, out-of-order, out-of-bounds, gapped, or incomplete ranges, descriptor mismatches, alternate canonical bytes, and unsupported namespaces before storage authority is returned; zero ranges remain compressed so a small hostile input cannot force one allocation per logical zero chunk. Raw-input and near-valid v2 decoder fuzz targets cover the parser. Product restore opens the selected disk/rootfs index over a sparse base when the flat cache is absent, verifies and promotes every lazy CAS source across the complete logical read before copying disk payload, and fails missing or corrupt multi-chunk, multi-descriptor requests by writing only the virtio I/O-error status; bundle pack holds the rootfs cache lock and copies only the descriptor-bound canonical index plus BLAKE3-verified objects, without treating the derived flat materialization as authority; bundle unpack/pull serialize shared-cache publication against GC, install verified objects before publishing their descriptor-bound index, and publish the completeness sidecar only after the full storage value is durable |
...
| `spore run` legacy and SPIO frames | guest vsock stream | bounded host buffers; the legacy/SPIO guest timing parser accepts only non-negative bounded integer fields, treats unknown or malformed values as absent telemetry, and is unit and fuzz covered, including named-exec user/system CPU; SPIO v1 frame headers, stream ids, payload lengths, flags, per-stream offsets, terminal output frames, and resize payloads are unit covered; malformed frames fail the run. The dedicated named-startup readiness request succeeds only after required rootfs and network setup and does not spawn a guest process. `start-v1` and `attach-v1` setup requests fail closed on unsupported stdio modes, and input attach validates the saved guest session had an interactive stdin pipe or PTY before accepting host bytes. TTY mode exposes a guest PTY to the child process and treats merged terminal bytes as untrusted guest output |
...
| Control socket JSON and named exec/copy SPIO stream | local consumers | local-only lifecycle monitor protocol is implemented for HVF and KVM, including fixed-RAM multi-vCPU create, exec, file copy, save, restore, and fork; named startup publishes ready metadata only after the guest agent answers the dedicated readiness request, then requires the socket to answer a `hello` request carrying the `spore.monitor.hello.v1` schema, an exact SporeVM version match, and the monitor helper contract before create, restore, or fork-child startup reports success, and control operations other than shutdown re-verify the same handshake; disk-backed children validate and adopt their baseline plus one-shot overlay fd before probing guest readiness; all normal, detached, interactive, and TTY named exec requests carry a bounded effective environment and absolute working directory, with image defaults merged against request-local overrides before the existing guest request is encoded; request-local values are not written to lifecycle or manifest state; all attached named exec and named file copy use streaming control requests plus the same bounded SPIO frame parser for optional stdin, terminal input, resize, separate stdout/stderr or merged terminal output, and exit; named exec sends one bounded monitor-generated timing event before exit, and local clients ignore unknown event types or reject malformed frame structure without granting guest authority; output frames use fail-fast local backpressure, aborting the exec when a disconnected or slow consumer cannot accept a complete frame within the 25 ms socket send deadline so the VMM/control loop cannot block indefinitely; monitor-generated guest session ids include a random per-process nonce so a restored guest cannot replay a prior monitor's cached request id, and host-side vsock ports advance from that random nonce for readiness and every request; detaching a completed stream drops queued packets for its old four-tuple before the next stream is attached; copy requests accept explicit regular files or directory trees, reject non-absolute guest paths plus `.`/`..` components, reject symlinks and special files, publish copy-in through no-overwrite temp paths, and create copy-out host destinations with no-overwrite flags; monitor processes deny child process execution through an embedded macOS sandbox profile or Linux seccomp filter; malformed requests fail closed and the socket lives in a private per-user `/tmp` namespace under a fixed-size digest of the resolved runtime root and full VM name. An adjacent owner symlink binds that digest to the readable private VM registry directory before monitor spawn, so collisions and ownership mismatches fail closed and cleanup cannot unlink another VM's socket |
...
| Retained initial-command output | guest workload stdout and stderr | Named create either discards output or continuously drains it into fixed 16,381-byte guest-agent buffers per stream. Excess bytes are discarded with explicit truncation flags, so an unattended child cannot block on a full pipe or grow host storage. The local monitor's `initial-output` request returns only that bounded snapshot through the existing framed collector, and removing the VM destroys the guest-resident bytes. |
| Saved lifecycle and exec-default metadata | input spore directories and bundles | Saved lifecycle fields are compatibility hints, not fresh host authority. In particular, `console_log_path` is untrusted and restore never opens or truncates it; named restore currently configures no console log until a future explicit restore option selects a new host path. Optional manifest `exec_defaults` and the compatible lifecycle copy are bounded to 64 environment entries and one absolute working directory before monitor startup. They contain image configuration only; per-exec overrides are never persisted. Ready, list, result, and failure output expose only the path actually configured for the live monitor. |
| Embedded initrd Toybox shell and applets | host-selected guest argv and guest workload input | the default initrd builds Toybox from pinned source with a minimal applet config and runs it only as guest child workload code; Toybox does not add host parsers, VMM devices, monitor control requests, or rootfs cache authority. Unsupported applets are absent and existing SporeVM helper binaries win over Toybox symlinks. Exact argv with a slash executes argv zero directly; slashless argv zero uses the shared builder/runtime guest resolver, bounded to the existing 250-byte PATH value, 64 entries, and a 512-byte candidate buffer. Missing or empty PATH fails without searching the working directory, executable relative matches fail closed, and absolute traversal cannot leave the guest VM after chroot. Procfs remains guest-wide and can name paths outside the image root just as an explicit slash-containing argv zero can; PATH lookup grants no host-filesystem authority. The selected executable path changes only the `execve` file argument; the original bounded argv is preserved. |
