...
| Native image archive | immutable CI or object-store artifact supplied to `spore image unpack` | The client requires the expected SHA-256 of the complete gzip archive and an explicit `linux/arm64` or `linux/amd64` platform before parsing. The strict USTAR reader accepts only the canonical manifest, config, index, sorted distinct object closure, and terminal zero blocks; it rejects alternate order, non-regular or extra entries, malformed headers, bad checksums, truncation, trailing bytes, aggregate bound violations, platform mismatch, native-image mismatch, and object length or BLAKE3 mismatch. All bytes stage privately outside the cache lock. The existing CAS transaction re-verifies objects and the index, marks complete storage, and publishes image metadata plus the local ref last. Header parsing has continuous fuzz coverage, and round-trip tests prove digest and architecture substitution leave no ref. The archive contains no saved-machine state and does not share the bundle parser or authority model. |
