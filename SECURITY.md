...
| `spore build` Dockerfile subset, `.dockerignore`, build-context hashing, and build executor control | local build contexts and caller-selected Dockerfiles; guest-originated RUN/COPY output after the first executor miss; host-originated COPY entry streams parsed by the guest agent | M1 supports only a fail-closed Dockerfile subset (`FROM`, shell-form `RUN`, flag-less `COPY`, `ENV`, `ARG`, `WORKDIR`, `CMD`) and named OCI-layout build contexts; unsupported syntax, context escape attempts, unsupported ignore patterns, unsupported COPY globs, missing sources, and special-file COPY inputs fail before cache lookup or image publication. Dockerfile and `.dockerignore` parsers carry fuzz targets in the introducing slice. The M2 executor uses the existing bounded SPIO v1 host parser for guest stdout/stderr/exit frames, fixed request shapes for `fsfreeze-v1`/`fsthaw-v1`, and rootfs-only snapshots only after the existing virtio-blk queue quiescence check; step-cache records are written only after the rootfs completeness stamp exists. `spore-copy-v1` is a host-produced, guest-parsed entry stream with bounded entry count, mode, path length, content length, and path component validation; the guest resolves apply paths from an fd for `/mnt/rootfs` with `openat2(RESOLVE_IN_ROOT | RESOLVE_NO_MAGICLINKS)` or a confined manual component walk on kernels without `openat2`; intermediate and final-component symlinks are followed only within the rootfs, so absolute targets re-root into the rootfs and cannot escape into the initrd namespace, without adding a tar parser or any new host-side guest-response parser |
