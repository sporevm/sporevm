...
| Bundle metadata, chunkpack index, pack segments, and local pull URIs | peers, registry, disk | `bundle.json`, `rootfs.index.json`, and chunkpack index parsing are fuzzed; unpack/pull only accept canonical metadata paths, canonical child ids, canonical pack paths, verified rootfs artifact paths, and absolute undecoded `file://` pull sources, then verify segment SHA256 plus logical BLAKE3 chunk IDs before writing chunks |
