...
| Immutable rootfs artifact resolution | manifest, local rootfs cache, bundle rootfs artifacts | product restore paths only accept the immutable ext4 rootfs kind, validate virtio-blk binding, and open the digest-addressed cache entry read-only under the verify-at-install, trust-at-open cache contract: the open refuses symlinked or non-regular entries and size mismatches without re-hashing installed bytes. Every install into the digest cache verifies content against the manifest digest and size before atomically publishing the entry read-only; user-supplied rootfs paths are always copied (never hardlinked) into the cache so later edits cannot alias cache entries; bundle unpack verifies exact rootfs bytes by manifest digest and size before installing them; metadata-only rootfs bundle policy is accepted only with an explicit materialization flag and a trusted digest-cache hit under the same local-cache contract |
| Manifest-bound chunked rootfs storage descriptor and disk index | manifest, registry, disk, bundle, peers | `rootfs.storage` is parsed as a separate storage/artifact authority from OCI provenance, requires `chunked-ext4-rootfs-v0`, BLAKE3, exact `rootfs/blake3` namespace, matching rootfs device binding, logical size matching the ext4 artifact size, and `base_identity == index_digest`; writable disks use `disk.kind: "chunk-index-disk-v0"` with the same descriptor fields and name their `spore-disk-index-v1` by `disk.base`; index bytes are BLAKE3-checked against the descriptor before parse, fuzzed, and rejected on unknown index kind, non-canonical ordering, duplicate or out-of-range chunks, implicit or overlapping zero-fill, descriptor mismatches, unsupported namespace, or malformed digests; the U1 parser accepts legacy `rootfs-block-index-v0` bytes only as a transition compatibility shape under the same descriptor and digest checks; product restore materializes the selected disk/rootfs index from BLAKE3-verified local chunk objects before guest use, and rootfs assembly additionally hashes the flat artifact and requires it to equal `rootfs.artifact.digest` before atomic publication; bundle unpack/pull install descriptor-bound indexes and chunks into the rootfs CAS cache through symlink-safe, verify-before-publish cache writes, and fail closed on missing or corrupt index/chunk bytes before guest use |
| Legacy writable disk layer indexes and disk objects | registry, disk | `disk-layer-v0` indexes remain a legacy parser surface during the chunk-index transition but are no longer produced by saves or opened by the runtime restore path; layer index bytes are verified by BLAKE3 ref before parse; disk objects are verified by BLAKE3 before reads in legacy helpers; extents and explicit zero clusters are canonical sorted, unique, and range-checked; corrupt or missing objects fail before use |
