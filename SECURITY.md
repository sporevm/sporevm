...
| x86-64 KVM PIO exit envelope | guest-issued x86 port I/O reflected through `KVM_EXIT_IO` | The host-only Stage 0a.1 boot harness validates the mapped `kvm_run` length, direction, width, decoded u16 port, nonzero count, checked aggregate byte length capped at 4KiB, data offset conversion, and the complete data range before reading or completing an exit. Malformed envelopes fail the harness and the decoder has focused malformed-input and continuous fuzz coverage. The provisional bring-up loop traces structurally valid ports, ignores writes, and returns zeroes for reads; this permissive trace policy is not reachable from the product runner and must be replaced by the native-evidence finite PIO table before the x86 board is frozen. |
| x86-64 KVM MMIO exit routing | guest-issued MMIO reflected through `KVM_EXIT_MMIO` | The host-only Stage 0a.2 boot harness requires a complete mapped `kvm_run` MMIO envelope, accepts only 1-, 2-, 4-, or 8-byte naturally aligned accesses wholly contained in one provisional virtio-mmio or generation-device window, and rejects every other address, width, overflow, boundary crossing, or truncated envelope before dispatch. The bounded target decoder has focused malformed-input and continuous fuzz coverage. This provisional router is not reachable from the product runner; Slice 2a must preserve these fail-closed bounds when the reviewed x86 board enters the product path. |
