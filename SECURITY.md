...
| Bundle metadata, chunkpack index, and pack segments | peers, registry, disk | `bundle.json`, `rootfs.index.json`, and chunkpack index parsing are fuzzed; unpack only accepts canonical metadata paths, canonical child ids, canonical pack paths, and verified rootfs artifact paths, then verifies segment SHA256 plus logical BLAKE3 chunk IDs before writing chunks |
