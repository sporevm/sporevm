...
| `spore-netd` frame stream, ARP, IPv4/UDP, and DNS parsers | guest-originated Ethernet frames via virtio-net | length-prefixed frame bounds, ARP reply handling, IPv4/UDP DNS dispatch, DNS name parsing, and malformed DNS handling are unit and fuzz covered; TCP and policy parsers must add focused tests or fuzzing when they land |
