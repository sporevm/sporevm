...
| x86-64 Linux direct-boot planner | host-selected bzImage, initrd, command line, and RAM size | The host-only Stage 0a.1 planner requires a complete Linux 2.10 setup header before reading its relocation fields, bounds every setup and payload offset, requires a nonzero `init_size` large enough for the loaded payload, and validates the relocatable-kernel alignment as a bounded power of two. It computes the separate load and runtime windows with the Linux boot protocol algorithm using checked arithmetic, bounds both within the single low-RAM slot, and places a nonempty initrd outside both. Command-line length excludes its separately checked terminator, absent or explicitly empty initrds produce zero ramdisk fields, and malformed inputs fail before guest RAM writes. Arbitrary raw images, header-derived placement fields, command lines, initrd lengths, and RAM sizes have continuous fuzz coverage alongside focused malformed-input tests. |
| x86-64 KVM PIO exit envelope and finite board policy | guest-issued x86 port I/O reflected through `KVM_EXIT_IO` | The host-only boot/profile harnesses validate the mapped `kvm_run` length, direction, width, decoded u16 port, nonzero count, checked aggregate byte length capped at 4KiB, data offset conversion, and the complete data range before reading or completing an exit. Their native-evidence policy accepts only one-byte, count-one accesses: writes of `0x0f` to CMOS index port `0x70`, writes of `0x0a` or `0x00` to CMOS data port `0x71`, reads of keyboard-controller status port `0x64` which return zero, writes of reset command `0xfe` to port `0x64` which terminate as `guest_reset`, and the exact `0x80 <- 0x00` legacy I/O-delay no-op observed while the Stage 0b.3 candidate initialized the in-kernel PIC. Direction, port, width, count, and value are all policy inputs; every other tuple fails the harness before exit completion. The bounded envelope and finite policy have focused malformed/nearby-input tests and continuous fuzz coverage. Raw `KVM_EXIT_SHUTDOWN` remains an unclassified fatal exit and cannot acquire reset or poweroff meaning. This harness is not reachable from the product runner; Slice 2a must preserve the same fail-closed table when the reviewed x86 board enters the product path. |
| x86-64 KVM MMIO exit routing and board lifecycle control | guest-issued MMIO reflected through `KVM_EXIT_MMIO` | The host-only Stage 0a.3 boot harness requires a complete mapped `kvm_run` MMIO envelope, accepts only naturally aligned 1-, 2-, or 4-byte accesses wholly contained in one provisional virtio-mmio window, permits the generation-device window to additionally accept aligned 8-byte accesses, and rejects every other address, width, overflow, boundary crossing, or truncated envelope before dispatch. The x86 board alone decodes its generation-page poweroff doorbell: an aligned 32-bit write of `0x46464f50` at offset `0x020` terminates as `guest_off`; any write touching that register with another width, placement, or value fails closed, while reads touching it return zero and have no side effect. The shared SPGN v1 device remains unchanged and inert for the board-owned control range. The bounded router and exact stateless control have focused malformed/nearby-input tests and continuous fuzz coverage, including 8-byte laundering rejection. This harness is not reachable from the product runner; Slice 2a must preserve these bounds and structured raw-exit evidence when the reviewed x86 board enters the product path. |
