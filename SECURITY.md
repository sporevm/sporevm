...
| Image gateway attachment records and subject lists | future authenticated gateway response | Canonical JSON attachment records and subject lists are each limited to 64 KiB and reject unknown or duplicate fields, alternate formatting, unsupported kinds or artifact types, malformed SHA-256 digests, noncanonical media types, invalid UTF-8, trailing input, unsorted descriptors, and duplicate descriptors. Artifact descriptors are nonempty and capped at 64 MiB; subject lists contain at most 256 descriptors. Verification binds a listed record's exact SHA-256 transport digest and artifact type to the immutable image-manifest subject before its descriptor can be used. Records and lists are data-only, carry fuzz coverage, enter no native image identity, create no mutable attachment-tag convention, and add no upload, storage, authorization, retention, or runtime authority. |
