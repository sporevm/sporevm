...
usage() {
  cat <<'EOF'
usage:
  test/remote/restore-leg.sh capture --backend kvm|hvf [--kernel Image] --initrd root.cpio --spore-dir DIR [options]
  test/remote/restore-leg.sh resume --backend kvm|hvf [--kernel Image] --spore-dir DIR [options]
  test/remote/restore-leg.sh same-host --backend kvm|hvf [--kernel Image] --initrd root.cpio [--workdir DIR] [options]
...
Examples:
  scripts/kernel/make-smoke-initrd.sh /tmp/sporevm-smoke.cpio
  test/remote/restore-leg.sh same-host --backend kvm --initrd /tmp/sporevm-smoke.cpio

  test/remote/restore-leg.sh capture --backend kvm --initrd /tmp/sporevm-smoke.cpio --spore-dir /tmp/kvm-spore
  tar -C /tmp -czf /tmp/kvm-spore.tgz kvm-spore
  # transfer kvm-spore.tgz directly to the HVF host, then:
  test/remote/restore-leg.sh resume --backend hvf --spore-dir /tmp/kvm-spore
EOF
}
...
if [[ -z "${kernel}" ]]; then
  kernel="$("${repo_root}/scripts/kernel/ensure-managed-kernel.sh" initrd)"
