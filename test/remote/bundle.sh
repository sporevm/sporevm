...
if [[ "\${source_network_smokes}" == "1" ]]; then
  SPORE_BACKEND=kvm test/smoke/network/config.sh
  SPORE_BACKEND=kvm test/smoke/network/dns.sh
  SPORE_BACKEND=kvm test/smoke/network/http.sh
  SPORE_BACKEND=kvm test/smoke/network/deny.sh
  SPORE_BACKEND=kvm test/smoke/network/capture.sh
fi
...
if [[ "\${writable_rootfs}" == "1" ]]; then
  ...
  mise exec -- env CC='zig cc -target aarch64-linux-musl' scripts/kernel/make-smoke-initrd.sh "\${workdir}/ticker.cpio"
  test/remote/restore-leg.sh capture \
    --backend kvm \
...
for iteration in \$(seq 1 "\${dest_repeat}"); do
  ...
  if [[ "\${writable_rootfs}" == "1" ]]; then
    ...
    test/remote/restore-leg.sh resume \
      --backend kvm \
...
if ((${#egress_gate_args[@]} > 0)); then
  "${repo_root}/scripts/internal/check-remote-bundle-egress.py" "${egress_gate_args[@]}" "${metrics_path}"
fi
