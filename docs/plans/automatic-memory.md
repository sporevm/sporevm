...
- Dirty RAM sealing is now shared in `src/dirty_ram.zig`, but initial seeding
  ...
  narrower seed set.
- Slice 1 is implemented on `lox/automatic-memory`: `boot.load` reports the
  kernel, initrd, and DTB ranges it wrote; the shared dirty RAM sealer accepts
  optional seed ranges; fresh KVM and HVF boot paths pass those ranges into
  dirty tracking. Resume paths deliberately keep the old full seed until a
  separate sparse-resume seed source exists.
...
Status: implemented locally and validated on HVF. KVM is wired in code but still
needs a Linux aarch64 KVM host run before treating the slice as fully landed.
