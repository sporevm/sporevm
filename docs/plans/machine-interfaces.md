...
- Slice 5 is active in this branch: `src/api.zig` exposes option-based
  ...
  named `rm`/`suspend`/`ls` commands through the API boundary instead of using
  command parsing as the product interface. Remaining CLI product paths should
  move behind the same boundary before the surface is considered complete.
