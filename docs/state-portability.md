...
| Immutable rootfs base | optional exact artifact plus optional `chunked-ext4-rootfs-v0` storage descriptor | yes via `spore run --image` | trusted flat-artifact open or verified chunked index/chunks | yes via `spore run --image` | trusted flat-artifact open or verified chunked index/chunks | product resume base; cache contract is verify-at-install, trust-at-open |
